...
export class HomeComponent implements OnInit { 
	...
	sendConversationMessage(message, next_message = false) {
		if(String(next_message) == 'enter_twitter_handle') {
			//manage twitter handle 
			//check message if it starts with @
			var startsWithTrue = message.startsWith("@");
			if(startsWithTrue == true) {
				//remove the @ and define handle

			} else {
				var handle = message;
			}
			//window.location.href = "/login";
			//call personality insights service
			// this._conversationService.callConversationService()
			// 	.subscribe(chat => {
